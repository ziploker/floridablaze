...
function Article({ userState }) {
	...
	const { data: storyFromRails, isLoading } = useQuery({
		queryFn: () => axios.get(`${params.id}/get_story_info_v2`).then((res) => res.data.story),
		queryKey: ["story"],
	})
	...
	return (
		<>
			...
			{Object.keys(userState.user).length > 0 && userState.user.isAdmin == true ? (
				<Link key={"b"} to={"/story_editor/" + storyFromRails.id} state={{ art: storyFromRails }}>
					edit STORY
				</Link>
			) : null}
			...
			<Helmet>
				...
				<link rel="canonical" href={`https://floridablaze.io/blog/${storyFromRails.slug}`}></link>
			...
			<ArticleSection>
				...
				<Caption dangerouslySetInnerHTML={{ __html: storyFromRails.caption }}></Caption>
				...
				<PWrapper dangerouslySetInnerHTML={{ __html: storyFromRails.body }}></PWrapper>

				<Comments userState={userState} artData={storyFromRails} slug={storyFromRails.slug} />
				{/* 
        <SideAds /> */}
			</ArticleSection>
		</>
