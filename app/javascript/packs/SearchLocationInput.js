...
function SearchLocationInput({
	...
	const handleKeyDown = (event) => {
		...
		if (event.keyCode == 13 || event.keyCode == 9) {
			console.log("inside Key Down Handler ================ enter or tab was hit")

			var firstValue = $(".pac-container .pac-item:first").text()

			if (firstValue.length > 0) {
				console.log("inside Key Down Handler ================ nothing was selected from the list")
				...
				autocomplete.getPlacePredictions(
					...
					function (predictions, status) {
						console.log("matching prediction is " + JSON.stringify(predictions[0]))
						if (status == "OK") {
							geocoder.geocode(
								{
			...
			console.log("Something that was not enter or tab was hit, key code(" + event.keyCode + ") ")
		}
	...
	async function handlePlaceSelect(updateQuery) {
		console.log("Inside google Change Event listiner =================== starttt")

		const addressObject = autoComplete.getPlace()

		if (addressObject.formatted_address) {
			console.log("ADDRESSS WAS SELECTED FROM GOOGLE LIST " + autoCompleteRef.current.value)
			...
			console.log("ENTER WAS HIT, BUT NO ADDRESS was SELECTED" + autoCompleteRef.current.value)
